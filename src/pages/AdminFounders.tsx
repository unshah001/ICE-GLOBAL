...
const AdminFounders = () => {
  ...
  const sections: AdminSectionLink[] = [{ id: "founders", label: "Founders" }];
  ...
  const updateDetail = (idx: number, key: keyof FounderDetail, value: any) =>
    setItems((prev) => {
      const next = [...prev];
  ...
  const saveCopy = async () => {
    ...
    try {
      ...
      if (!token) throw new Error("Not authenticated.");
      const attempt = async (authToken: string) =>
        fetch(`${base}/founders/detail-copy`, {
          method: "PUT",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${authToken}` },
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      ...
      setError(err.message || "Unable to delete founder");
    }
  };
  ...
  return (
    <AdminLayout
      ...
      description="Manage founder profiles, highlights, and detail pages."
      navItems={navItems}
      sections={sections}
    >
      {error && (
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Explore more founder profiles"
            />
          </div>
          <div>
            ...
            <Input
              ...
              placeholder="Scroll to reveal more founders..."
            />
          </div>
        </div>
        <div className="flex justify-end gap-2">
      ...
      <div className="grid gap-4">
          ...
          <Card key={item.id} className="bg-card/80 border-border/70">
            <CardHeader className="flex flex-row items-center justify-between">
              <div className="space-y-1">
                <CardTitle>{item.name || "New founder"}</CardTitle>
                <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                  {item.era && <Badge variant="secondary">{item.era}</Badge>}
            ...
            <CardContent className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input value={item.id} onChange={(e) => updateItem(idx, "id", e.target.value)} placeholder="founder-1" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input value={item.era} onChange={(e) => updateItem(idx, "era", e.target.value)} placeholder="ICE 1.0" />
                </div>
                <div>
        ...
        {!items.length && <p className="text-sm text-muted-foreground">No founders yet. Add your first founder to begin.</p>}
        {hasMore && (
