...
const CofounderDetail = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Cofounder fetch failed");
        const data = (await res.json()) as CofounderItem;
        setCofounder(data);
      } catch {
        setCofounder(null);
      } finally {
    ...
    const loadMore = async () => {
      try {
        const res = await fetch(`${base}/cofounders/list?limit=12`);
        if (!res.ok) throw new Error("List fetch failed");
        const data = await res.json();
        setMoreCofounders((data.data || []).filter((f: CofounderItem) => f.id !== id));
      } catch {
        setMoreCofounders([]);
      }
    };
  ...
  const story = cofounder.detail || {
    headline: cofounder.name,
    summary: cofounder.highlight,
    heroImage: cofounder.image,
    highlights: [],
    metrics: [],
  };
  const heroImg =
    resolveMedia(
      pickVariant(story.heroVariants, ["main", "hero", "medium"]) ||
        story.heroImage ||
        pickVariant(cofounder.variants, ["main", "medium", "thumb"]) ||
        cofounder.image
    ) || "";
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-[70vh] flex items-end pb-16 pt-28 md:pt-36 overflow-hidden">
        ...
        <div className="container-custom relative z-10 space-y-6">
          <Link to="/cofounders" className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {cofounder.social?.linkedin && (
              <Button asChild variant="secondary" size="sm">
                <a href={cofounder.social.linkedin} target="_blank" rel="noreferrer">
                  LinkedIn
                </a>
              </Button>
            )}
            {cofounder.social?.twitter && (
              <Button asChild variant="outline" size="sm">
                <a href={cofounder.social.twitter} target="_blank" rel="noreferrer">
                  Twitter
                </a>
              </Button>
            )}
            {cofounder.social?.website && (
              <Button asChild variant="ghost" size="sm">
                <a href={cofounder.social.website} target="_blank" rel="noreferrer">
                  Website
                </a>
              </Button>
            )}
          ...
          <div className="max-w-4xl space-y-4">
            <h1 className="text-4xl md:text-6xl font-display font-bold leading-tight">{story.headline || cofounder.name}</h1>
            <p className="text-lg md:text-xl text-muted-foreground">{story.summary || cofounder.highlight}</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="container-custom grid lg:grid-cols-3 gap-10 lg:gap-14 items-start">
                    ...
                    >
                      ...
                      <div className="relative h-full p-6 flex items-end">
                        <p className="text-lg text-foreground/90 leading-relaxed">
                          {story.pullQuote || cofounder.highlight}
                        </p>
                      </div>
                    </motion.div>
                  </div>
          ...
          <div className="space-y-6 sticky top-28">
            <div className="glass rounded-2xl p-6 border border-border/70 space-y-4">
              ...
              {(story.summary || cofounder.highlight) && (
        ...
        <div className="container-custom space-y-8">
          ...
          <StickyScrollReveal
                  ...
                  <div className="relative z-10 flex flex-col gap-3 h-full justify-end text-left text-white p-6">
                    ...
                    <div className="flex items-center justify-between text-xs text-white/70">
                      ...
                      <Button asChild variant="secondary" size="sm" className="rounded-full px-4 py-2">
                        <Link to={`/cofounders/${item.id}`}>View co-founder</Link>
                      </Button>
                    </div>
                  </div>
                </div>
            ...
            onItemSelect={(item) => {
              ...
              if (target) navigate(`/cofounders/${target}`);
            }}
          />
        </div>
      </section>
...
export default CofounderDetail;
